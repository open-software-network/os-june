...
use june_api::{ApiLimits, ApiState, ApiStateParams};
use june_config::{AppConfig, ModelPriceConfig, ModelProvider};
use june_providers::{
...
fn build_router(
    ...
    let os_accounts: Arc<dyn june_domain::OsAccountsClient> = Arc::new(
        OsAccountsHttpClient::from_config(http.clone(), &config.os_accounts),
    );
    let transcriber: Arc<dyn june_domain::Transcriber> = Arc::new(
    ...
    let token_verifier: Arc<dyn june_domain::TokenVerifier> = Arc::new(
    ...
    let _ = tracing_subscriber::registry()
        .with(
            tracing_subscriber::EnvFilter::try_from_default_env().unwrap_or_else(|_| {
                "june=info,june_api=info,june_services=info,june_providers=info,tower_http=info".into()
            }),
        )

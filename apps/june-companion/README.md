...
presentation, never tokens, private/session keys, APNs tokens, pairing secrets,
or raw protocol frames.

June Companion starts by scanning or entering a five-minute pairing code
created by a signed-in June Desktop. It generates its own revocable device
credential and can use it only after that Mac approves the credential hash and
device identity. The companion has no account login or bearer token, the

...
| JUN-306 (pre-PR) | Standards (codex, r1) | 5 | 5 | caught glossary vocabulary, ambiguous proxy naming, and mutable agent-run terminology across code, tests, and docs; all fixed before the final audit |
| JUN-306 (pre-PR) | Spec (codex, r1) | 6 | 5 | smoke false-pass, unlocked media dispatch, steer model collapse, failed-new-session loss, and note-chat scope were real; raw legacy local-ID precedence was deliberate compatibility behavior |
| JUN-306 (pre-PR) | Adversarial (codex, iterative) | 11 | 11 | found live-model acknowledgement races, media-lock stalls, multi-steer snapshot loss, CLI continuation drift, incomplete Auto aliases, cold slash retargeting, fast FIFO terminal loss, and note-switch/crash-consistency races |
| JUN-306 (pre-PR) | Adversarial (claude, r1) | 5 | 4 | found media lock/catalog fallback/provisional persistence; the general idle retry budget was bounded recovery, not an unbounded stall |
| JUN-306 (pre-PR) | Integration (#732/#733 codex) | 4 | 4 | caught the keepOpen/target-options collision, global-vs-session Auto preference scope, stale optimistic cost-quality ref, and the need to rebase onto both merged UI PRs |
| JUN-306 (pre-PR) | Final standards (codex) | 4 | 4 | explicit remote/local display collision, Note Chat legacy provenance upgrade, merged-UI reconciliation, and ambiguous runtime-session naming all survived verification |
| #734 | Standards (codex, r1+final) | 5 | 5 | tightened agent-run, June, proxy, and stored/runtime session vocabulary across implementation, tests, and docs; final convergence clean |
| #734 | Spec (codex, iterative) | 6 | 5 | caught fresh Note Chat initialization, mixed follow-up ordering, note-rebind submission state, completion-before-preparation deadlock, and steer deletion cleanup; legacy local-ID precedence remained deliberate compatibility behavior |
| #734 | Adversarial (codex, iterative) | 11 | 11 | found first-run global picker leakage, true Send-time FIFO gaps, stop-before-create leakage, queued steer/media reservation loss, completion/preparation races, deletion leaks across every reservation owner, and cross-session consent ownership |

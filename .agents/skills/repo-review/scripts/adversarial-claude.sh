#!/usr/bin/env bash
# Run the adversarial review through Claude Code (headless `claude -p`, read-only).
# Usage: adversarial-claude.sh [-C <worktree>] [-f <focus>] [-o <out.md>] [--dry-run] [<fixed-point>]
...
#   -o <out.md>    file for the final verdict; default: stdout only
...
# Read-only: git history commands allowlisted, edit tools disallowed outright;
# everything else permission-gated defaults to denied in -p mode.
cd "$worktree"
run_claude() {
  printf '%s\n' "$prompt" | claude -p \

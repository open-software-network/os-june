...
would race the active agent run and its tool loop.
...
and the on-device provider proxy must distinguish an explicitly remote model from a
...
- The model picker remains available while an agent run is active. Choosing a
...
- June stores reserved internal Hermes model ids for Auto, explicit remote, and
  local routes. The on-device provider proxy decodes those ids before forwarding
...
- Changing the picker during an agent run is immediate in the UI but deliberately
  has no effect on that agent run. The next prompt is blocked if its captured
  model cannot be applied; June never silently falls back to the previous one.
- A `message.complete` frame is not proof that Hermes is idle. Post-run work or
...
- The reserved model-id prefixes are an internal Hermes/provider-proxy
...
- The June API wire contract does not change and no June API deployment is
  required.
...
- **Interrupt and restart the active agent run on the new model.** Rejected: a picker

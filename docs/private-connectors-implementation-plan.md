...
- **Account routing:** local mode supports multiple connected Google accounts, with one Keychain item and one non-secret index row per identity. Read requests without an explicit identity fan out across the connected accounts and merge results. Mutations require an explicit account; June asks when the user has not made the identity clear, and approval cards name the acting email. Event triggers and autonomous grants persist their selected `account_id`; routine creation and editing default only when exactly one Google account is connected and otherwise require an explicit choice. Existing routine bindings are preserved without migration. Linear and GitHub remain single-account.

# Reproducible builds for `clovy-api`
...
so `repo` becomes `…/clovy-api@sha256`, the manifest URL 404s, the prelaunch
...
### 1. Pin base images by digest — `clovy-api/Dockerfile`
...
### 2. Deterministic compiler flags — `clovy-api/Dockerfile` (builder stage)
...
### 4. Reproducible image export — `.github/workflows/build-clovy-api.yml`
...
Usage: verify-reproducible.sh <git-ref>
  ...
  2. SOURCE_DATE_EPOCH=$(git -C /tmp/verify log -1 --pretty=%ct) \
       ...
       --metadata-file /tmp/meta.json /tmp/verify/clovy-api   # build the worktree, NOT cwd
  ...
  4. sha=$(git -C /tmp/verify rev-parse --short=7 HEAD)
     expected=$(docker buildx imagetools inspect "ghcr.io/open-software-network/clovy-api:$sha" \

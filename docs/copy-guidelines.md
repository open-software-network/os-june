...
- **E2EE:** the prompt is encrypted on your device and only decrypted inside a hardware-secured enclave, and the response is encrypted before it leaves the enclave; no prompt data is ever readable by the model provider or its infrastructure.
...
**What the company stores:** account, login, and billing records, via OS Accounts. **What it never stores:** prompts, transcripts, files, memory. The one exception is content the user explicitly sends us: an in-app issue report carries the user's description, the agent's diagnosis, and any attachments to the team's tracker, and says so in the flow. Never present the storage claim as covering things a user deliberately submits. (Source: `privacy.promises` in `site-copy.ts`; onboarding "June doesn't collect your data" step; `scribe-api/crates/api/src/handlers/issues.rs`.)

**The agent's two risk surfaces, never to be conflated:** inference privacy is June's property and applies to every model June suggests. Action risk is governed by layers the user controls: by default the agent works inside a sandbox that blocks writes outside its own workspace, it asks before running anything risky, every session has a full activity log, and it can be stopped at any moment. When the user approves an outward action (sending an email, visiting a site), the other side sees what it shares. June keeps your data private; it cannot make the rest of the internet private. (Source: the onboarding honesty step, `src/components/onboarding/steps/LearnSteps.tsx`.)
...
| "The agent is safe", "June won't make mistakes" | The product's own onboarding says the opposite, on purpose. Honesty about fallibility is brand canon | "The agent can make mistakes. So June keeps it on a short leash: sandboxed by default, approvals for anything risky, a full activity log, and you can stop it at any moment." | `LearnSteps.tsx` honesty step |
...
- **"Anything risky waits for your yes."** The agent approval model in one line. (Its predecessor "Nothing changes until you say yes" is retired: workspace writes and metered turns do not prompt, so it overclaimed.)
...
- **The honesty-step triad** ("The agent can make mistakes." / "So June keeps it on a short leash." / "Private inference protects your data. It doesn't approve the agent's actions."). Quote the headings exactly as in `LearnSteps.tsx`; the template for all agent-risk copy.
...
- **Upstream provider** is a third-party AI service reached through Scribe API. Avoid: vendor, AI provider, and (in technical docs) model provider. "Model provider" is the one approved consumer-copy term for the third party in the model tier descriptions, matching `model-privacy.ts`; CONTEXT.md records the same exception.

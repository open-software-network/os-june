...
June is a private AI assistant for the Mac, made by OpenSoftware. It does three things: dictation (hold a key, speak, and clean text lands at your cursor in any app), meeting notes (records and transcribes meetings, then writes structured notes), and an agent (hands off real desktop tasks, built on the open source Hermes framework). Requires macOS 14+, an OpenSoftware account (OS Accounts), and a subscription with a free trial.
...
**Stays on the Mac:** the agent runtime (a local Hermes process), its files, sessions, memory, and state (`src-tauri/src/hermes_bridge.rs`); recordings, transcripts, and notes (local app database); dictation history (local, with a retention window).

**Leaves the Mac, only for model inference:** audio (for transcription) and prompts plus context (for note generation, dictation cleanup, and the agent's thinking). Nothing else is mirrored anywhere.
...
- **E2EE:** the prompt is encrypted on the Mac and only decrypted inside a hardware-secured enclave; no prompt data is ever readable by the model provider or its infrastructure.
...
|---|---|---|---|
| "100% local", "everything runs on your Mac", "nothing ever leaves your device" | Inference leaves the Mac by design: audio and prompts go out for every model call | "June runs locally. Prompts and audio leave your Mac only for model inference, through private routing." | `README.md:19` |
...
| "Your meetings never leave your Mac" | Meeting audio leaves for transcription (that is how notes get made); transcripts and notes are stored locally | "Transcripts and notes stay on your Mac. Audio goes out only for transcription, through private routing." | `README.md:19`, CONTEXT.md (note transcription) |

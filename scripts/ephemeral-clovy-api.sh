#!/usr/bin/env bash
# Deploy the working-tree clovy-api to a disposable Phala CVM, use it, delete it.
#
...
# Copy a Clovy key from clovy-api/.env, falling back to its June-era alias. A
# missing or empty key stays absent. The sealed-env parser takes values raw,
# while dotenvy strips quotes and trailing comments locally, so fail fast
...
copy_compat_env_line() {
  ...
  case "$line" in
    *\"* | *\'* | *" #"*)
      die "$API_ENV_FILE: $canonical is quoted or has a trailing comment; use a plain KEY=value line (sealed env takes values raw)." ;;
  esac
  value="${line#*=}"
  printf '%s=%s\n' "$canonical" "$value"
}
...
cmd_up() {
  ...
  image="ttl.sh/clovy-api-eph-$(uuidgen | perl -ne 'print lc'):4h"
  ...
  name="clovy-api-eph-$(whoami | perl -pe 'chomp; $_ = lc; s/[^a-z0-9]+/-/g')-$(openssl rand -hex 4)"
  token="$(openssl rand -hex 32)"
  created="$(date -u +%Y-%m-%dT%H:%M:%SZ)"

  echo "Building clovy-api for linux/amd64 and pushing to $image ..."
  ...
  # single-arch manifests. Same rationale as .github/workflows/build-clovy-api.yml.
  docker buildx build \
  ...
  IMAGE_REF="$image" perl -0pe 's/\$\{(?:CLOVY|JUNE)_IMAGE\}/$ENV{IMAGE_REF}/g' "$COMPOSE_FILE" >"$rendered"

  {
    printf 'CLOVY__LOCAL_DEV__ENABLED=true\n'
    printf 'CLOVY__LOCAL_DEV__BEARER_TOKEN=%s\n' "$token"
    printf 'CLOVY__LOCAL_DEV__USER_ID=%s\n' "$LOCAL_DEV_USER_ID"
    copy_compat_env_line CLOVY__UPSTREAMS__VENICE__API_KEY JUNE__UPSTREAMS__VENICE__API_KEY
    copy_compat_env_line CLOVY__UPSTREAMS__OPENAI__API_KEY JUNE__UPSTREAMS__OPENAI__API_KEY
  } >"$env_file"
...
Or use this one from a manual session (token is in $STATE_FILE, mode 600):
  export CLOVY_API_URL=$url
  export OS_CLOVY_LOCAL_DEV=1
  export CLOVY_DEV_SKIP_LOCAL_API=1
  export OS_CLOVY_LOCAL_DEV_BEARER_TOKEN="\$(jq -r .token $STATE_FILE)"
...
cmd_dev() {
  ...
  CLOVY_API_URL="$url" \
    OS_CLOVY_LOCAL_DEV=1 \
    OS_CLOVY_LOCAL_DEV_BEARER_TOKEN="$token" \
    OS_CLOVY_LOCAL_DEV_USER_ID="$LOCAL_DEV_USER_ID" \

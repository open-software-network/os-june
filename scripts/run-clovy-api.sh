...
if [[ -n "$existing_pids" ]]; then
  while IFS= read -r pid; do
    ...
    if [[ "$cwd" == "$API_DIR" ]]; then
      echo "clovy-api is already running from this workspace on port $PORT (pid $pid)." >&2
      exit 0
    fi
    ...
    echo "Stop that process or set CLOVY_API_PORT to a different port." >&2
    exit 1
  done <<<"$existing_pids"
...
cd "$API_DIR"
exec cargo run -p clovy-api-server -- serve

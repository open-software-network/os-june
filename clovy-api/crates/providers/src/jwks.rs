...
use clovy_config::OsAccountsConfig;
use clovy_domain::{AuthError, TokenVerifier, UserId};
use jsonwebtoken::{
...
mod tests {
    ...
    async fn accepts_valid_es256_token() -> Result<(), Box<dyn Error>> {
        ...
        Mock::given(method("GET"))
            ...
            .mount(&server)
            .await;
        let verifier = build_verifier(&server.uri(), "issuer", "clovy-api");
        let token = test_token("ec01", "usr_123", "issuer", "clovy-api")?;
    ...
    async fn requires_the_requested_scope() -> Result<(), Box<dyn Error>> {
        ...
        let verifier = build_verifier(&server.uri(), "issuer", "clovy-api");
        let token =
            test_token_with_scope(["ec01", "usr_123", "issuer", "clovy-api"], "profile:read")?;
    ...
    async fn unknown_kid_does_not_amplify_jwks_refreshes() -> Result<(), Box<dyn Error>> {
        ...
        Mock::given(method("GET"))
            ...
            .await;
        let verifier = build_verifier(&server.uri(), "issuer", "clovy-api");
        let bad_token = test_token("kid_unknown", "usr_123", "issuer", "clovy-api")?;
    ...
    async fn rejects_wrong_issuer() -> Result<(), Box<dyn Error>> {
        ...
        Mock::given(method("GET"))
            ...
            .await;
        let verifier = build_verifier(&server.uri(), "issuer", "clovy-api");
        let token = test_token("ec01", "usr_123", "other-issuer", "clovy-api")?;

...
// Compile-time budget contracts with clovy-config's image route arithmetic.
//
...
const _: () = assert!(
    (CHARGE_RETRY_ATTEMPTS as u64) * crate::http::DEFAULT_TIMEOUT_SECS
        ...
        <= clovy_config::IMAGE_SETTLEMENT_TIMEOUT_MARGIN_SECS
);
...
const _: () = assert!(
    crate::http::DEFAULT_TIMEOUT_SECS <= clovy_config::OS_ACCOUNTS_AUTHORIZE_TIMEOUT_BUDGET_SECS
);
...
mod tests {
    ...
    fn p3a_sink_from_config_is_disabled_without_token() {
        let mut config = clovy_config::AppConfig::default().os_accounts;

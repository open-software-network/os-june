use crate::{
    ApiResponse, CLOVY_APP_VERSION_HEADER, CLOVY_MACOS_VERSION_HEADER, LEGACY_APP_VERSION_HEADER,
    LEGACY_MACOS_VERSION_HEADER, client_header, state::ApiState,
};
use axum::{Json, extract::State, http::HeaderMap};
use clovy_config::ComputerUseConfig;
use serde::Serialize;
...
pub(crate) async fn rollout(
    ...
    let clovy_version = client_header(
        &headers,
        CLOVY_APP_VERSION_HEADER,
        LEGACY_APP_VERSION_HEADER,
    );
    let macos_version = client_header(
        &headers,
        CLOVY_MACOS_VERSION_HEADER,
        LEGACY_MACOS_VERSION_HEADER,
    );
...
fn rollout_decision(
    ...
    let reason = if !config.enabled {
        Some("global")
    } else if version_is_disabled(clovy_version, &config.disabled_clovy_versions) {
        // Released clients may log this response value, so it remains a
        // compatibility wire value even though the config field is Clovy.
        Some("june_version")
...
mod tests {
    ...
    fn rollout_can_be_killed_globally_or_by_either_version() {
        ...
        config.disabled_clovy_versions = vec!["0.0.*".to_string()];
        let clovy = rollout_decision(&config, "0.0.33", "15.5.1");
        assert!(!clovy.enabled);
        assert_eq!(clovy.reason, Some("june_version"));

        config.disabled_clovy_versions.clear();
    ...
    fn missing_or_nonmatching_versions_do_not_trigger_a_scoped_rule() {
        let config = ComputerUseConfig {
            disabled_clovy_versions: vec!["0.0.32".to_string()],

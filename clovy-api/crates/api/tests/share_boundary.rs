...
use clovy_services::{ShareService, ShareServiceDeps};
...
fn share_router() -> Router {
    ...
    clovy_api::router(state_with_share(Some(service)))
}

fn state_with_share(share: Option<Arc<ShareService>>) -> clovy_api::ApiState {
...
async fn share_endpoints_answer_501_until_configured() {
    let router = clovy_api::router(state_with_share(None));
...
async fn viewer_router_exposes_reads_but_not_owner_or_product_routes() {
    let router = clovy_api::viewer_router(state_with_share(None));

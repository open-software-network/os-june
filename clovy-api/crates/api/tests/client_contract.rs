...
//! docs/adr/0021-clovy-api-v1-compatibility-policy.md for the rules and for
...
fn error_code_registry_is_stable() -> Result<(), Box<dyn Error>> {
    ...
    let current = BTreeMap::from(
        [
            ("badRequest", clovy_api::ERR_BAD_REQUEST),
            ("unauthorized", clovy_api::ERR_UNAUTHORIZED),
            ("unprocessable", clovy_api::ERR_UNPROCESSABLE),
            ("notFound", clovy_api::ERR_NOT_FOUND),
            ("insufficientCredits", clovy_api::ERR_INSUFFICIENT_CREDITS),
            ("payloadTooLarge", clovy_api::ERR_PAYLOAD_TOO_LARGE),
            ("authorizationDenied", clovy_api::ERR_AUTHORIZATION_DENIED),
            ("internal", clovy_api::ERR_INTERNAL),
            ("upstream", clovy_api::ERR_UPSTREAM),
            ("metering", clovy_api::ERR_METERING),
            ("serviceOverloaded", clovy_api::ERR_SERVICE_OVERLOADED),
            ("timeout", clovy_api::ERR_TIMEOUT),
            ("sharingUnavailable", clovy_api::ERR_SHARING_UNAVAILABLE),
        ]
...
async fn error_envelope_shape_is_stable() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(
        body["error_code"],
        serde_json::json!(clovy_api::ERR_UNAUTHORIZED)
    );

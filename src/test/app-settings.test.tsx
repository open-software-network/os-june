...
describe("AppSettings", () => {
  ...
  it("changes the agent privacy guard mode from Agent settings", async () => {
    ...
    await user.click(screen.getByRole("tab", { name: "Agent" }));
    const privacyRow = screen.getByText("Privacy guard").closest(".settings-row");
    expect(privacyRow).not.toBeNull();
    const controls = within(privacyRow as HTMLElement);

    expect(controls.getByRole("button", { name: "Off" })).toHaveAttribute("aria-pressed", "true");

    await user.click(controls.getByRole("button", { name: "Structured" }));
    expect(localStorage.getItem(AGENT_PRIVACY_GUARD_MODE_KEY)).toBe("structured");
    expect(controls.getByRole("button", { name: "Structured" })).toHaveAttribute(
      "aria-pressed",
      "true",
    );

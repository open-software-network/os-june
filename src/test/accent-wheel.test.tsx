...
describe("AccentWheel", () => {
  ...
  it("keeps the trigger dot persistent while the wheel closes", () => {
    ...
    expect(within(openGroup).getByRole("radio", { name: "Rose" })).toHaveAttribute(
      "aria-checked",
      "true",
    );
    expect(within(openGroup).getByRole("radio", { name: "Clay" })).toHaveAttribute(
      "aria-checked",
      "false",
    );
    ...
    expect(screen.getByRole("button", { name: "Accent color: Blue. Change" })).toBeInTheDocument();
    expect(
      screen
    ...
    expect(within(closingGroup).getByRole("radio", { name: "Rose" })).toHaveAttribute(
      "aria-checked",
      "true",
    );
    expect(within(closingGroup).getByRole("radio", { name: "Blue" })).toHaveAttribute(
      "aria-checked",
      "false",
    );
  ...
  it("waits to commit the app theme until the wheel has closed", () => {
    ...
    expect(screen.getByRole("radiogroup", { name: "Accent color" })).toHaveAttribute(
      "data-phase",
      "closing",
    );

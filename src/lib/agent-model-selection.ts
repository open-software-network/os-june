...
export const AGENT_RUN_AUTO_MODEL_PREFIX = "__clovy_auto_generation__:";
const LEGACY_AGENT_RUN_AUTO_MODEL_PREFIX = "__june_auto_generation__:";
...
export function agentModelSelection(modelId: string): AgentModelSelection {
  const prefix = [AGENT_RUN_AUTO_MODEL_PREFIX, LEGACY_AGENT_RUN_AUTO_MODEL_PREFIX].find((value) =>
    modelId.startsWith(value),
  );
  if (!prefix) return { modelId };
  const encoded = modelId.slice(prefix.length);

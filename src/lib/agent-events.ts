...
export const AGENT_SESSION_ID_ROTATED_EVENT = "scribe:agent:session-id-rotated";
...
export type AgentSessionIdRotatedDetail = {
  oldSessionId: string;
  newSessionId: string;
...
export function dispatchAgentSessionIdRotated(
  detail: AgentSessionIdRotatedDetail,
) {
  window.dispatchEvent(
    new CustomEvent<AgentSessionIdRotatedDetail>(
  ...
  void import("@tauri-apps/api/event")
    .then((api) =>
      typeof api.emit === "function"
        ? api.emit(AGENT_SESSION_ID_ROTATED_EVENT, detail)
        : undefined,
    )
    .catch(() => {});
}
